...
V1.1 在保留 V1.0 USB 主图像帧和上位机显示链路的基础上，把采样侧重构为固定 `25` 颗 ADS1220 轮询：

`MCU -> USB -> Host -> 10x10 图像显示`

拓扑固定为 `5` 块子板、每块 `5` 颗 ADS1220、每颗 `4` 路单端输入。像素映射固定为 `pixel_id = device_id * 4 + channel_id`，正常图像输出使用 `FRAME_TYPE_FULL_REAL`，仍保持 `420` 字节 `frame_packet_t` 外部格式。

当前工程采用“`CubeMX` 负责底层外设初始化，应用层负责状态机调度”的分层方式。系统主流程如下：

`初始化 -> 偏置稳定 -> 25 设备 scheduler 轮询 -> 100 pixel 缓冲 -> TIM6 输出许可 -> USB 发送`

ADS1220 默认工作在 `turbo + 2000 SPS + continuous conversion`。V1.1 使用共享 SPI 总线和单活动设备轮询；`ADC_DRDY_MUX` 只表示当前被选择 ADS1220 的 DRDY。设备切换后会清理旧 DRDY 事件，EXTI 事件也会按当前 `device_id/channel_id` 绑定校验。

V1.1 上电初始化阶段不会逐颗 ADS1220 执行 SPI 配置或 link_check。`APP_STATE_INIT` 只做共享硬件动作和软件表准备：`adc_bus_reset_all()`、`adc_bus_start_all_pulse()`、`ads1220_device_table_init()`、`ads1220_scheduler_start()`。其中 `ads1220_device_table_init()` 只是初始化 25 个软件设备对象，填充路由和默认 `expected_config`，不是逐颗访问芯片。真正的 SPI 配置发生在运行期 scheduler 轮到某个 `device_id/channel_id` 时：`SELECT_DEVICE -> ENSURE_CONFIG` 会调用 `ads1220_device_configure_channel()`，再通过 `adc_protocol_configure()` 配置当前被选中的 ADS1220。
...
- `Core/Src/app.c`
  应用状态机核心，负责系统启动流程、scheduler 服务、恢复策略、时间戳和 USB 打包
- `Core/Src/board_topology.c`
  固定 25 设备拓扑和 `pixel_id = device_id * 4 + channel_id` 映射
- `Core/Src/adc_bus.c`
  ADS1220 GPIO/SPI 总线层，收口 `SUB_SEL`、`ADC_SEL`、`ADC_CS_GATE`、`ADC_RST_ALL`、`ADC_START_ALL` 和 `ADC_DRDY_MUX`
- `Core/Src/ads1220_device.c`
  固定 25 个设备对象，每个设备独立保存 expected_config 和最近采样状态
- `Core/Src/ads1220_scheduler.c`
  单活动 ADS1220 轮询状态机，遍历 25 设备 x 4 通道并写入 100 pixel buffer
- `Core/Src/adc_protocol.c`
  ADS1220 协议层，负责 `RESET / START/SYNC / POWERDOWN / WREG / RREG / 读 3 字节 / 补码解析`，不直接操作 GPIO
...
- `Core/Src/frame_builder.c`
  测试帧、半真实帧、占位帧和 V1.1 full-real 帧构造
- `Core/Src/diag.c`
...
V1.1 保留历史状态枚举以兼容诊断帧，但采样细节已下放到 `ads1220_scheduler`。当前长期运行态主要在 `APP_STATE_WAIT_DRDY` 中推进 scheduler，scheduler 内部执行 `SELECT_DEVICE -> ENSURE_CONFIG -> START_OR_ARM -> WAIT_DRDY -> READ_SAMPLE -> STORE_RESULT -> ADVANCE_DEVICE`。

这也意味着设备配置是“谁被轮到，谁当场配置”。系统不会在上电阶段先把 25 颗 ADS1220 全部依次 reset/config/link_check 完再进入运行态；启动期只建立固定 25 设备的软件拓扑，实际 SPI 事务始终围绕当前活动设备展开。
...
- `APP_STATE_RECOVER`
  按细分故障码执行自动恢复动作，例如 SPI 重试或当前 device/channel 的 ADC reset/config/link_check
- `APP_STATE_FAULT`
  停止采样，周期性发送故障状态帧；只有连续恢复失败或不可恢复初始化错误才进入

## 状态转换流程

V1.1 的真实执行路径已经简化为 `INIT -> BIAS_STABILIZE -> WAIT_DRDY/RECOVER/USB_FLUSH/FAULT`，采样细节由 `ads1220_scheduler` 内部状态机推进。下面的 V0.2/V1.0 状态转换说明保留为历史背景；V1.1 固定 25 设备轮询细节以 `docs/v1_1_multi_device.md` 为准。
...
V1.1 图像输出由固定 25 设备 scheduler 驱动：完成 100 pixel 扫描且 TIM6 输出许可可用时，构造并发送 1 个 `FRAME_TYPE_FULL_REAL` 10x10 图像帧。
...
`ADC_CS_GATE` 空闲态必须为高；`adc_bus_cs_assert()` 拉低，`adc_bus_cs_deassert()` 拉高。上层业务代码不直接写 `ADC_CS_GATE` 电平。
...
- 项目固定 25 设备，不实现任意 N 泛化框架
- 不做多 DRDY 并发中断；只服务当前被 MUX 选中的 ADS1220
- 自动恢复已接入 V1.1 scheduler：DRDY timeout/SPI/config mismatch 会进入 `RECOVER`，连续失败后进入 `FAULT`
...
- 固定 25 设备轮询说明：`docs/v1_1_multi_device.md`
- 新增分层：`board_topology`、`adc_bus`、`ads1220_device`、`ads1220_scheduler`
- 正常图像帧类型：`FRAME_TYPE_FULL_REAL`
- USB CDC 命令入口：`I/P/B` 由 `CDC_Receive_FS()` 转发到 `app_on_usb_command_rx()`
